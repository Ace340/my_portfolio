...
- **Calm zone** — a section where the Live Proof field is intentionally quiet so foreground content owns the eye. Two forms: *occluded* (an opaque backing hides the field entirely — e.g. the hero, `#contact`) and *dimmed* (a transparent backing where the field stays visible but its intensity is reduced while the section is in view — e.g. About). Distinct from a *transparent section*, which shows the field at full intensity.
- **Scroll-story** — a multi-beat narrative revealed progressively with scroll, one beat at a time. Distinct from the *motion spine* (the site-wide smooth-scroll + easing substrate): the motion spine carries *all* scrolling, a Scroll-story carries *meaning* (its beats). Strictly subordinate to the *Signature Moment* — motion in service of content, never a second spectacle. Opposed to a *static positioning block* (one paragraph, no progression).
